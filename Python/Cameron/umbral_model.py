...
rM,m,tau,TM,V,nH,ne,pM,beta,rhoM = np.loadtxt("Maltby_E.txt",unpack=True)
rMM,mM,tauM,TMM,VM,nHM,neM,pMM,betaM,rhoMM = np.loadtxt("Maltby_M.txt",unpack=True)
rML,mL,tauL,TL,VL,nHL,neL,pML,betaL,rhoML = np.loadtxt("Maltby_L.txt",unpack=True)
...
def plot_p():
  with plt.style.context(['science']):
    ...
    plt.grid()
    plt.savefig("IMAGES/umbral_density.png",dpi=300)
    plt.show()
def plot_rho():
  with plt.style.context(['science']):
    x = rM-550-70
