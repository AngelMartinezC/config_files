...
# -- constants and limits for interpolation of Model S
G_const = 6.67e-11  # SI
RSun = 695.99036e3 # [km]
MSun = 1.989e33   # [g]
r1 = RSun*(r0-1)
...
#exit() #stop just for print the constants
...
# -- Gravity interplation in range [-1.5e3,0] from Newton's 2nd law
fgravity = interp1d(-1*RSun*(1-r2g),g_gravity)
xx3 = np.linspace(0,min(xx1),len(xx1))

SI1 = 1e3*1e3*1e1  # convert to international system
der1 = np.gradient(prs1,xx1)
der2 = np.gradient(prs_int(xx1),xx1)


# ---------------------------------------------------------------------------
# -- Read gravity from Model GONG Charlie
...
xc0 =  1*np.ones(len(r0c))
xc1 = -1500*np.ones(len(r0c))
#print(-RSun*r0c)
#plt.plot(-RSun*r0c)
#plt.show()
sc0 = np.argwhere(np.diff(np.sign(xc0+RSun*r0c)))
sc1 = np.argwhere(np.diff(np.sign(xc1+RSun*r0c)))
idxc0, idxc1 = sc0[0][0], sc1[0][0]+2  # To get more in depth (+2)

# -- Interpolation for Charlie gravity
gexp = 2
var_g = np.polyfit(-RSun*r0c[idxc0:idxc1],gc[idxc0:idxc1]/1e2,gexp)
g_int = np.poly1d(var_g)
print(g_int)
plt.figure(figsize=(6,4))
plt.subplot(121)
intep = "$a="+latex_float(g_int[0])+"$\n$b="+latex_float(g_int[1])+"$\n$c="+latex_float(g_int[2])+"$"
plt.plot(-RSun*r0c[idxc0:idxc1]/1e3,gc[idxc0:idxc1]/1e2,'x-',label='Model l4b.14a')
plt.plot(-RSun*r0c[idxc0:idxc1]/1e3,g_int(-RSun*r0c[idxc0:idxc1]),
    label='Interpolation $\mathcal{O}(x^{%d})$ \n%s'%(gexp,intep),
    color='limegreen',lw=2)
plt.grid()
plt.legend(frameon=True)
plt.ylabel(r'gravity $(-\nabla p/\rho)~[\text{m s}^{-2}]$',labelpad=5)
plt.xlabel(r'$\text{Depth} ~[\text{Mm}]$')

plt.subplot(122)
plt.axhline(y=0,color='k')
plt.plot(-RSun*r0c[idxc0:idxc1]/1e3,(g_int(-RSun*r0c[idxc0:idxc1])*1e2-gc[idxc0:idxc1])/gc[idxc0:idxc1]*100*1e4,label=r'$\% $ error $(\times 10^{4})$')
plt.xlabel(r'$\text{Depth} ~[\text{Mm}]$')
plt.legend(frameon=True)
plt.grid()
plt.ylim(-5,5)
plt.tight_layout()
plt.savefig('grav_interp.png',dpi=300)
...
plt.plot(xx3/1e3,fgravityc(xx3), '--', label=r'$g$ Charlie Model l4b.14a', 
    lw=2,c='r')
#plt.plot(nr/1e3, (nrho+0.02)*UNIT_PRESSURE/(UNIT_LENGTH*UNIT_DENSITY)/1e2, 'o-.',
plt.plot(-nr/1e3, (nrho)*UNIT_PRESSURE/(UNIT_LENGTH*UNIT_DENSITY)/1e2, '.-',lw=1,
    ms=2,label='PLUTO simulation',color='m')
plt.plot(xx1/1e3,g_int(xx1),label=r'$g$ Interpolation $\mathcal{O}(x^{%d})$'%gexp,
    color='k',ls='--')
